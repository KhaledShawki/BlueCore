...
Bool AcquireSemaphoreFor( Semaphore& semaphore, TimeDuration timeout ) noexcept
{
  ...
  const DWORD result =
    ::WaitForSingleObject( GetNativeSemaphore( semaphore.NativeHandle ), Windows::ToTimeoutMilliseconds( timeout ) );
...
Bool ReleaseSemaphore( Semaphore& semaphore, Uint32 count ) noexcept
{
  ...
  return ::ReleaseSemaphore( GetNativeSemaphore( semaphore.NativeHandle ), static_cast< LONG >( count ), nullptr ) != 0;
}
} // namespace Blue
