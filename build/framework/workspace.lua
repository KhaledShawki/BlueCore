...
function bb.workspace(desc)
    ...
    local outputRoot = bb.get_build_output_root()
    workspace(desc.name)
    location(path.join(outputRoot, "build/" .. (_ACTION or "none")))
    ...
    staticruntime("Off")
    targetdir(path.join(outputRoot, "bin/%{cfg.system}/%{cfg.platform}/%{cfg.buildcfg}"))
    objdir(path.join(outputRoot, "obj/%{cfg.system}/%{cfg.platform}/%{cfg.buildcfg}/%{prj.name}"))
